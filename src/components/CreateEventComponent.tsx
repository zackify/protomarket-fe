...
export function CreateEventComponent() {
  ...
  const createEvent = async () => {
    ...
    try {
      ...
      const startTimeUTC = BigInt(Math.floor(new Date(startDate).getTime() / 1000));
    ...
    <div className="bg-gray-900/30 rounded-lg p-6 border border-green-400/20">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          />
          <p className="text-xs text-gray-500 mt-1 font-mono">Default: 0% (0-25% allowed)</p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 bg-gray-800 border border-green-400/20 rounded-lg text-white font-mono text-sm focus:outline-none focus:border-green-400/50 focus:ring-1 focus:ring-green-400/20"
          />
          <p className="text-xs text-gray-500 mt-1 font-mono">Event will start at the specified time</p>
        </div>
        ...
        <div className="pt-4">
          <button
            onClick={createEvent}
            disabled={isPending || !isConnected || !outcomeA.trim() || !outcomeB.trim()}
